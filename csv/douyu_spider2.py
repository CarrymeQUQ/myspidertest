...
class DouyuSpider:
    ...
    def save_content_list(self, content_list):
        ...
        # 把文件存成csv格式的文件，header写出列名，index写入行名称
        # mode="a" 循环遍历追加到文件中
        # 遍历加上header属性
        df = pandas.DataFrame(data=content_list)
        df.to_csv("douyu2.csv", index=False, header=False, sep=',', encoding="utf-8-sig", mode="a")
    ...
    def run(self):
        ...
        # 读取csv文件
        douyu_csv= pandas.read_csv('douyu2.csv')
        print(douyu_csv)
        # 5.点击下一页的元素 循环
